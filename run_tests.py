...
def dofile_result_same_with_ref(dofile, args):
    ...
    run_qsyn = subprocess.Popen(
        ["./qsyn", "-f", dofile], stdout=PIPE, stderr=STDOUT, env={"OMP_WAIT_POLICY": "passive"})
...
def update_test_refs(dofiles, args):
    ...
    results = threads.starmap(update_dofile_ref, zip(
        dofiles, itertools.repeat(args)))
...
def main():
    ...
    for path in args.paths:
        if os.path.isfile(path):
            ...
            dofiles.extend(glob.glob(os.path.join(
                path, "**/*.dof"), recursive=True))
        else:
            print(f"Warning: ignoring path '{path}': not a file nor a dir!!")

    if args.file:
        for dofile in dofiles:
            subprocess.run(["./qsyn", "-f", dofile])
        return 0
    if args.update:
