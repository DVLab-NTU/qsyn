...
dvlab::BooleanMatrix::Row matmul_gf2(dvlab::BooleanMatrix const& matrix, dvlab::BooleanMatrix::Row const& vec) {
    DVLAB_ASSERT(matrix.num_cols() == vec.size(), "matmul_gf2: dimension mismatch");
    auto out = dvlab::BooleanMatrix::Row(matrix.num_rows(), 0);
    for (size_t r = 0; r < matrix.num_rows(); ++r) {
        unsigned char acc = 0;
        for (size_t c = 0; c < matrix.num_cols(); ++c) {
            acc ^= static_cast<unsigned char>(matrix[r][c] & vec[c]);
        }
        out[r] = acc;
    }
    return out;
...
[[nodiscard]] int get_z_y_candidate_v3(dvlab::BooleanMatrix const& phase_poly_matrix, dvlab::BooleanMatrix const& l_matrix, std::unordered_map<dvlab::BooleanMatrix::Row, std::vector<std::pair<int, int>>, dvlab::BooleanMatrixRowHash> const& s_matrices, dvlab::BooleanMatrix::Row& max_y, dvlab::BooleanMatrix::Row& max_z) {
    ...
    for (auto it = s_matrices.begin(); it != s_matrices.end(); ++it) {
        ...
        // Why choose index=0?
        auto y = matmul_gf2(b_matrix_hstack, augmented_nullspace_transposed[0]);
...
std::pair<StabilizerTableau, Polynomial> FastToddReferencePhasePolynomialOptimizationStrategy::optimize(StabilizerTableau const& clifford, Polynomial const& polynomial) const {
    if (polynomial.empty()) {
...
std::pair<StabilizerTableau, Polynomial> TohpeOnlyPhasePolynomialOptimizationStrategy::optimize(StabilizerTableau const& clifford, Polynomial const& polynomial) const {
    if (polynomial.empty()) {
        fmt::println("Polynomial is empty, returning the input Clifford and polynomial");
        return {clifford, polynomial};
    }
    ...
    if (std::ranges::any_of(polynomial, [](PauliRotation const& rotation) { return 4 % rotation.phase().denominator() != 0; })) {
        spdlog::error("Failed to perform TOHPE-only optimization: the polynomial contains a non-4th-root-of-unity phase!!");
        return {clifford, polynomial};
    }
    ...
    spdlog::trace("Polynomial before TOHPE-only:\n{}", fmt::join(ret_polynomial, "\n"));
    spdlog::debug("num_terms before TOHPE-only: {}", ret_polynomial.size());
    ...
    while (true) {
        ...
        spdlog::trace("Polynomial after TOHPE-only:\n{}", fmt::join(ret_polynomial, "\n"));
        spdlog::debug("num_terms after TOHPE-only: {}", ret_polynomial.size());
    }
    ...
    if (auto clifford_ops = multi_linear_polynomial.extract_clifford_operators(); clifford_ops.has_value()) {
        ...
        spdlog::error("Failed to perform TOHPE-only optimization: the post-optimization polynomial does not have the same signature as the pre-optimization polynomial!!");
        return {clifford, polynomial};
    }
