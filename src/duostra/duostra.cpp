...
bool Duostra::map(bool use_device_as_placement) {
    ...
    auto device_copy = _device;
    auto router = std::make_unique<Router>(std::move(_device), cost_strategy, DuostraConfig::TIE_BREAKING_STRATEGY);
    auto est_router = std::make_unique<Router>(std::move(device_copy), cost_strategy, DuostraConfig::TIE_BREAKING_STRATEGY, DuostraConfig::SCHEDULER_TYPE == SchedulerType::astar);
    ...
    if (if_astar()){
        fmt::println("A* Scheduler Routing...");
