...
void ZXOPTimizer::printSingle(const string& rule) {
    if (_rules.find(rule) != _rules.end()) {
        cout << rule << "(r2r, s2s): (";
        if (getR2R(rule) == INT_MAX)
            cout << "INF";
        else
            cout << getR2R(rule);
        cout << ", ";
        if (rule == "Interior Clifford Simp" || rule == "Clifford Simp")
            cout << "-";
        else if (getS2S(rule) == INT_MAX)
            cout << "INF";
        else
            cout << getS2S(rule);
        cout << ")" << endl;
...
bool ZXOPTimizer::updateParameters(ZXGraph* g) {
    ...
    if (verbose >= 8) cout << "# " << _lVerticeCnt << " vertices, " << _lEdgeCnt << " edges, " << _lTCnt << " T-gates, Density = " << _lDensity << " -> Stop:" << stop << endl
                           << endl;
    return stop;
}