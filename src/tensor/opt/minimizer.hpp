...
enum class MinimizerKind { CoordinateDescent, LBFGS };

inline std::unique_ptr<Minimizer> make_minimizer(MinimizerKind kind) {
    switch (kind) {
        case MinimizerKind::LBFGS:             return std::make_unique<LBFGSMinimizer>();
        case MinimizerKind::CoordinateDescent: return std::make_unique<CoordinateDescentMinimizer>();
    }
